...
- Keep the coordinator loop running through the configured Orca scheduled prompt (`*/5 * * * *`, Asia/Tokyo). Each invocation runs `pnpm run orchestration:coordinator -- --once`, consumes settled worker results, and then stops; the loop is the only component allowed to create the next Dispatch.
