...
- Poll through the configured Orca scheduled prompt (`*/5 * * * *`, Asia/Tokyo) with one reused existing-workspace session. Each tick runs exactly one `pnpm run orchestration:coordinator -- --once` sweep and exits; the schedule is the poller and must not create overlapping coordinator sessions or Dispatches. A persistent `--loop` is reserved for explicit foreground recovery and must not be started by the scheduler. The coordinator is the only component allowed to create the next Dispatch.
- The coordinator must use `orca orchestration worker-start` for every new worker. Do not recreate the old `worktree create` plus low-level `dispatch --inject` combination for normal tasks: it can leave a visible Dispatch without a supervised worker, causing `unknown_dispatch` completion failures. If a start is failed or indeterminate, inspect the returned effects and exact Orca/Git worktree state before any retry.
...
- Select up to three remaining leaf tasks in priority and ordinal order, using the bounded `selectedTasks` batch from `pnpm run backlog:dispatchable`. If no leaf task remains, report that no dispatchable task exists and do not create a Run, Dispatch, worktree, or worker. Do not add artificial dependencies or change a parent status solely to hide it from dispatch candidates.
- Worktree creation is single-flight per task and must be polled to completion. Even in a parallel batch, create and poll each requested worktree one at a time; after its final JSON result, the worker sessions may run concurrently. If `orca-ide worktree create ... --json` returns a session handle or continues without final JSON, poll that exact command session until it exits. Empty output, a timeout, or `runtime_unavailable` is indeterminate, not permission to retry; inspect `orca-ide worktree list` and `git worktree list` first, and never issue a second create for the same task/name while the first request may still be running.
...
- Reconcile completion rows with `orca orchestration worker-list` by orchestration task ID: `worker-start` task rows may omit `dispatch_id`, while the settled Dispatch mapping remains in `worker-list`. Do not skip a completed worker solely because its task row lacks that field.
- Treat task IDs from dispatched, completed, failed, and blocked worker rows as reserved across all Runs. Do not automatically retry a failed or blocked task; resume it only through an explicit recovery action.
- A retained, uncertain, manual-review, or dirty-branch row must be reported and skipped while the single coordinator loop continues polling other workers; it must not stop the session or trigger an automatic retry.
- Before sending `worker_done`, the worker must commit and push the task-record change and all scoped work on its task branch. A clean, pushed branch is required for the coordinator to create the one Draft PR; an uncommitted or unpublished result is retained for recovery and does not consume the next task slot.
