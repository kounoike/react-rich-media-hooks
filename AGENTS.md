...
- Keep the coordinator loop running through the configured Orca scheduled prompt (`*/5 * * * *`, Asia/Tokyo) with one reused existing-workspace session. That session starts `pnpm run orchestration:coordinator -- --loop`; the process owns the single-flight lock and polls settled worker results every five minutes. Schedule ticks must not create additional coordinator sessions or Dispatches. The loop is the only component allowed to create the next Dispatch.
