...
- Keep the coordinator loop running through the configured Orca scheduled prompt (`*/5 * * * *`, Asia/Tokyo). Each invocation starts `pnpm run orchestration:coordinator -- --loop`; the first process owns the single-flight lock and polls settled worker results every five minutes, while later scheduled invocations exit if that loop is still alive. The loop is the only component allowed to create the next Dispatch.
