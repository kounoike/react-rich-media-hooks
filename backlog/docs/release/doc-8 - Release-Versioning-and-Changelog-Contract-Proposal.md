---
id: doc-8
title: 'Release, Versioning, and Changelog Contract Proposal'
type: specification
created_date: '2026-08-30 21:21'
updated_date: '2026-08-30 21:25'
tags:
  - release
  - versioning
  - changelog
  - proposal
---
# Release, Versioning, and Changelog Contract Proposal

## Status and authority

This is an approval-bound proposal for TASK-1.21. It is not an accepted release, versioning, compatibility, distribution, dependency, or automation decision. The user must explicitly approve or revise the recommendation before TASK-1.23 configures release automation or before any version, tag, GitHub Release, or package publication is treated as governed by this contract.

The proposal inherits accepted decision-1 (product and quality contract) and decision-2 (public API and resource lifecycle). It also respects the approval-bound compatibility/distribution proposal in doc-7 and proposed decision-3. Exact package entry points, browser minima, asset packaging, module conditions, and supply-chain policy remain subject to those approval gates and are not silently resolved here.
...
The leading automation recommendation is **tagpr as the version/release authority, with GitHub's generated release-notes configuration feeding the release PR and changelog**. Release Drafter is not a second version authority and is not required for the first release. If a continuously updated notes preview is later valuable, Release Drafter may be used as a complementary, draft-only presentation layer whose resolved version, tag, package versions, and publication state are ignored; it must not create a competing tag or release. This recommendation remains pending explicit user approval.

The recommendation is deliberately compatible with one package today and coordinated package/asset manifests later. TASK-1.23 may select an equivalent implementation only after the user approves the contract and records any required Decision; it must preserve the observable ordering, review gates, immutable checkpoints, and recovery rules below.
...
All options below are evaluated, not accepted. Any tool choice is deferred to TASK-1.23 after contract approval and the toolchain/CI/supply-chain tasks.
...
The following are approval-bound requirements for the eventual workflow:
...
## 8. Open approval questions

The user must explicitly choose or revise the following before TASK-1.23 implementation:

- tagpr as the sole routine release authority versus Changesets or release-please, and whether Release Drafter is omitted or allowed only as a draft-only complementary preview;
- the initial package version and whether the repository starts at `0.y.z` or another user-selected version;
- GitHub PR release labels/body-note schema versus Changesets or Conventional Commit metadata as the required change-intent input;
- exact number/roles of normal, major, security, and emergency approvers;
- the `CHANGELOG.md` categories, whether a top `Unreleased` section is retained, and whether generated release notes may include contributor lists;
- the exact single-package and any future companion-package/asset tag naming and coordinated-release policy;
- npm trusted publishing/OIDC and provenance as the required publication path, plus the protected environment name and emergency fallback;
- whether draft GitHub Releases are always created before package publication and published only after registry verification;
- the minimum prerelease channels, promotion rules, deprecation window, and pre-1.0 breaking-change policy; and
- whether the release evidence gate includes the full browser/quality matrix for every release or only for support-boundary/effect changes.

Until these questions are answered, this proposal must remain unaccepted and automation must not encode them as settled public or distribution behavior.
...
- [doc-7 — Compatibility and Distribution Contract Proposal](../distribution/doc-7 - Compatibility-and-Distribution-Contract-Proposal.md)
...
| Task criterion | Evidence in this proposal |
| --- | --- |
...
| #5 Alternatives and explicit approval | This status section, Section 4, and Section 8 make the proposal and all tool/contract choices approval-bound; no accepted Decision is created or implied. |
